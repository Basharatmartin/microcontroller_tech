...
void __leds_toggle(){
	...
	for(;;) {		// forever loop
		...
		if (serialRead() != -1){
			if ((P1IN & 0x20) == 0x20) 	// checking the P1.5 if it is ON / OFF and displaying it using serialPrintln
				serialPrintln("LED: ON");
			else
				serialPrintln("LED: OFF");
		}
		
		__delay_cycles(187688);	// 5c00 ms = 187688  !!! this will work on windows - code composer by using __delay_cycles(500000);
	}
}
